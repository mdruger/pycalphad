...
cdef class Problem:
    ...
    def parameter_jacobian(self, x_in, parameters_in):
        cdef CompositionSet compset = self.composition_sets[0]
        cdef int phase_idx = 0
        cdef int num_statevars = len(compset.phase_record.state_variables)
        cdef int var_offset = num_statevars
        ...
        cdef double[::1] x = np.array(x_in)
        cdef double[::1] parameters = np.array(parameters_in)
        ...
        for compset in self.composition_sets:
            ...
            compset.phase_record.parameter_gradient(grad_tmp, x, parameters)
            compset.phase_record.parameter_jacobian(jac_view, x, parameters)
            for dof_x_idx in range(num_statevars):
    ...
    def chemical_potential_parameter_gradient(self, x_in, parameters_in):
        "Assuming the input is a feasible solution."
        # mu' = (A+)' grad + (A+) hess
        jac = self.mass_jacobian(x_in).T
        jac_pinv = np.linalg.pinv(jac)
        param_jac = self.parameter_jacobian(x_in, parameters_in)
